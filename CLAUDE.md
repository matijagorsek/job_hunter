...
- **Environment variables for all runtime configuration** (7 projects)
...
- **Monitoring / notification as primary use case** (5 projects)
...
- **Telegram used as primary notification channel** (this project)
...
- **Fix branches stall indefinitely with hundreds of dirty uncommitted files** (seen here)
...
- **Critical security findings survive 9–17 cycles without remediation** (seen here)
...
- **Errors silently swallowed or masked by catch-all handlers** (5 projects)
...
- **Public endpoints with zero authentication** (seen here)
...
- **No authentication on public-facing endpoints** (seen here)
...
- **Critical findings persist for 10+ cycles without remediation** (seen here)
...
- **Critical security findings persist unresolved for 10+ cycles** (seen here)
...
- **No startup env-var validation — silent NaN/undefined failures** (4 projects)
...
- **Missing or unenforced authentication on public-facing endpoints** (seen here)
...
- **No startup env-var validation — silent undefined at runtime** (4 projects)
...
- **No startup validation of required environment variables** (4 projects)
...
- **Fix branches stall indefinitely with growing dirty file counts** (seen here)
...
- **Fix branches created but never committed** (seen here)
...
- **Fix branch switched away from without committing — work lost** (seen here)
...
- **Credentials and secrets leaked into git history** (seen here)
...
- **Fix branches opened but never merged** (seen here)
...
- **Fix branches abandoned mid-work across multiple projects** (seen here)
...
- **package-lock.json uncommitted — non-reproducible installs** (3 projects)
...
- **Credentials committed to or leaked through version control** (seen here)
...
- **Missing or uncommitted lockfiles allow non-deterministic installs** (3 projects)
